...
echo "$(date) $line git pull" >> /media/Library\SPE_Automated/indexing-logs/git.log && git --git-dir=/opt/lib/collections/.git --work-tree=/opt/lib/collections pull 1>> /media/Library\SPE_Automated/indexing-logs/git.log 2>&1
