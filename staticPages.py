...
affixHead.set("src", "js/headerAffix.js")
...
alphaContent = alpha.find("body/div/div[@id='mainContent']")
...
apapContent = apap.find("body/div/div[@id='mainContent']")
...
gerContent = ger.find("body/div/div[@id='mainContent']")
...
mssContent = mss.find("body/div/div[@id='mainContent']")
...
uaContent = ua.find("body/div/div[@id='mainContent']")
...
subjectsContent = subjects.find("body/div/div[@id='mainContent']")
...
def makePanel(id, title, date, extent, abstract, link):
	...
	if "cubic ft." in extent.lower():
		span.text = " " + str(extent) + " (about " + str(extent.split(" ")[0]) + " boxes)"
	else:
		span.text = " " + str(extent)
	p.text = abstract
...
def makeLink(collection, pageContent):
	...
	if collection[1] == "True":
		...
		if id.startswith("apap"):
			...
			link = "https://archives.albany.edu/browse/ger.html#" + id
		elif id.startswith("mss"):
			link = "https://archives.albany.edu/browse/mss.html#" + id
		elif id.startswith("ua"):
			link = "https://archives.albany.edu/browse/ua.html#" + id
...
def alphaNav(contentDiv, collection):
	...
	alphaContent = contentDiv.find("div[@class='row no-gutter']/div[@class='col-md-9 col-md-offset-3 alphaContent']/div[@class='appendContent']")
...
for subjectRow in sortedSubjects:
	...
	rightContent = subjectsContent.find("div[@class='row no-gutter']/div[@class='col-md-9 col-md-offset-3 alphaContent']/div[@class='appendContent']")
	...
	subjectLink.set("href", "https://archives.albany.edu/browse/" + subjectNumber + ".html")
	...
	if subject == "Death Penalty":
		...
		pageContent = page.find("body/div/div[@id='mainContent']")
	...
	elif subject.strip().lower().startswith("ualbany"):
		page = copy.copy(tempate)
		page.find("head/title").text = subject
		pageContent = page.find("body/div/div[@id='mainContent']")
		pageTemplateFile = os.path.join(staticDir, "templates", "browseUASubjects.xml")
		pageTempateInput = ET.parse(pageTemplateFile, parser)
		pageTempate = pageTempateInput.getroot()
	...
	for collection in sortedCollections:
		...
		if collection[0] in subjectRow:			
			...
			rightContent = pageContent.find("div[@class='row no-gutter']/div[@class='col-md-9 col-md-offset-3 alphaContent']/div[@class='appendContent']")
			letter = collection[2][:1]
			
			
			if subject.strip().lower().startswith("ualbany"):
				for navLink in leftNav:
					if navLink.tag == "div":
						for nextLink in navLink:
							if not nextLink.find("a") is None:
								if "href" in nextLink.find("a").attrib:
									if subjectNumber + ".html" in nextLink.find("a").attrib["href"]:
										nextLink.attrib["class"] = "list-group-item active"
										if not navLink.getprevious() is None:
											navLink.getprevious().attrib["class"] = "list-group-item active"
					else:
						if not navLink.find("a") is None:
							if "href" in navLink.find("a").attrib:
								if subjectNumber + ".html" in navLink.find("a").attrib["href"]:
									navLink.attrib["class"] = "list-group-item active"
			else:
				if leftNav.find("li/a[@id='link-" + letter + "']") is None:
					navLi = ET.Element("li")
					navLink = ET.Element("a")
	...
	pageString = ET.tostring(page, pretty_print=True, method='html', xml_declaration=False, doctype="<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">")
	pageFile = open(os.path.join(staticDir, subjectNumber + ".html"), "wb")
	pageFile.write(pageString)
	pageFile.close()
	

def stringEscape(bytesInput):
	byteString = bytesInput.replace(b"&lt;", b"<").replace(b"&gt;", b">").replace(b"emph", b"i")
...
apapString = ET.tostring(apap, method='html', xml_declaration=False, doctype="<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">")
apapFile = open(os.path.join(staticDir, "apap.html"), "wb")
apapFile.write(stringEscape(apapString))
apapFile.close()

gerString = ET.tostring(ger, method='html', xml_declaration=False, doctype="<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">")
gerFile = open(os.path.join(staticDir, "ger.html"), "wb")
gerFile.write(stringEscape(gerString))
gerFile.close()

mssString = ET.tostring(mss, method='html', xml_declaration=False, doctype="<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">")
mssFile = open(os.path.join(staticDir, "mss.html"), "wb")
mssFile.write(stringEscape(mssString))
mssFile.close()

uaString = ET.tostring(ua, method='html', xml_declaration=False, doctype="<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">")
uaFile = open(os.path.join(staticDir, "ua.html"), "wb")
uaFile.write(stringEscape(uaString))
uaFile.close()

subjectsString = ET.tostring(subjects, method='html', xml_declaration=False, doctype="<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">")
subjectsFile = open(os.path.join(staticDir, "subjects.html"), "wb")
subjectsFile.write(stringEscape(subjectsString))
subjectsFile.close()

alphaString = ET.tostring(alpha, method='html', xml_declaration=False, doctype="<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">")
alphaFile = open(os.path.join(staticDir, "alpha.html"), "wb")
alphaFile.write(stringEscape(alphaString))
alphaFile.close()